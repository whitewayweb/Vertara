...
| Text overlays and emoji stickers | Add any number of independently timed text layers or local emoji stickers. Every layer supports editable text, foreground colour, optional transparent background, six social-friendly type styles, fade/pop/slide entrance motion, size, draggable in-video position, and edge-resizable width. Blank layers are not exported. | `features/project/text-overlays.ts`, `components/molecules/video-hook-overlay.tsx` | Yes |
...
| Social safe-area guides | Toggle an overlay that marks a conservative text-safe region while editing. | `components/atoms/social-safe-area-guides.tsx` | Preview only; never exported |
