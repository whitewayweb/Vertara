...
export function PosterVideoPreview({ className, headline, playback, sourceUrl, subline }: PosterVideoPreviewProps) {
  return (
    <div className={cn("flex h-full flex-col gap-3 bg-gradient-to-br from-violet-700 via-fuchsia-600 to-orange-400 p-3 text-white", className)}>
      <div>
      ...
      <PlaybackVideo className="min-h-0 flex-1 rounded-md object-contain shadow-lg" playback={playback} sourceUrl={sourceUrl} />
    </div>
  );
}
