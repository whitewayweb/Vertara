...
**Current status:** the browser editor now guides users through comparing output layouts, choosing a layout, and editing it alongside shared trim, speed, mute, and colour-adjustment controls. Creators can add any number of text layers or local emoji stickers from the compact toolbar above the preview; every layer has direct text, colour, optional transparent fill, six social-friendly type styles, fade/pop/slide-up entrance motion, size, timing, in-video positioning, and edge-resizable width controls shared by Canvas, Focus, Poster previews, and MP4 export. Brightness, contrast, saturation, and warmth are applied to every layout in preview and export. Preview playback loops within the selected trim range. Rendering remains DOM-video based while the adaptive canvas renderer and export pipeline are still deferred.
